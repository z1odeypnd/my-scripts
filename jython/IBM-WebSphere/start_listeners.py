      ...
      if AdminConfig.showAttribute(eachAppserver, 'serverType') == 'APPLICATION_SERVER':
        ...
        try:
          for eachListenerPort in AdminConfig.list('ListenerPort', eachAppserver).split():
            ...
            listenerPort = AdminControl.queryNames('type=ListenerPort,cell=%s,node=%s,process=%s,name=%s,*' % (cellName, nodeName, serverName, portName))
            ...
            try:
              ...
              print 'Cell=%s, Node=%s, AppSrv=%s, LP_Name=%s, LP_State=%s' % (cellName, nodeName, serverName, portName, portState)
              checkingPort = ''
            except:
              continue
        except:
